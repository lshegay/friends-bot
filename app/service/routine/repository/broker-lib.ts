...
import type { RoutinesRepositoryTasks as RoutinesUsecaseRepositoryTasks } from '../usecase/repository';
...
export class RoutinesRepositoryTasks implements RoutinesUsecaseRepositoryTasks {
  ...
  async getTasks(): Promise<Ok<RoutineTask[]> | Err<Error>> {
    const messages = this.deps.mb.getMessages();
