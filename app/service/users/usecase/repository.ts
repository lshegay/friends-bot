...
export interface UsersRepository {
  ...
  updateUser: (userId: string, user: Partial<Omit<User, 'id'>>) => Promise<Ok<User> | Err<Error>>;
  deleteUser: (id: string) => Promise<Ok<void> | Err<Error>>;
}

export interface UsersTelegramRepository {
  getChatMember: (chatId: number, userId: number) => Promise<Ok<User> | Err<Error>>;
}
